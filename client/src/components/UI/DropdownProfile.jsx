/* eslint-disable no-undef */
import React, { useRef, useState } from 'react';
import {TransitionGroup, CSSTransition, } from "react-transition-group";
function DropdownProfile({items}) {
    const [show, setShow] = useState(false)
    const nodeRef = useRef(null);
 
    return (
        <div className='relative'>
             <button 
             className='bg-gray-400 h-14 w-14 p-1 rounded-full hover:bg-amber-50'
             onClick={()=> setShow(!show)}
             >
            <svg viewBox="-3.36 -3.36 30.72 30.72" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0" transform="translate(2.3999999999999986,2.3999999999999986), scale(0.8)"><rect x="-3.36" y="-3.36" width="30.72" height="30.72" rx="15.36" fill="#e0e5e6" strokewidth="0"></rect></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fillRule="evenodd" clipRule="evenodd" d="M2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12ZM12 6C10.3431 6 9 7.34315 9 9C9 10.6569 10.3431 12 12 12C13.6569 12 15 10.6569 15 9C15 7.34315 13.6569 6 12 6ZM17.8946 17.4473L17.0002 17C17.8946 17.4473 17.8939 17.4487 17.8939 17.4487L17.8932 17.4502L17.8916 17.4532L17.8882 17.46L17.88 17.4756C17.8739 17.4871 17.8666 17.5004 17.858 17.5155C17.8409 17.5458 17.8186 17.5832 17.7906 17.6267C17.7346 17.7138 17.6558 17.8254 17.5497 17.9527C17.3369 18.208 17.0163 18.5245 16.5549 18.8321C15.6228 19.4534 14.1751 20 12.0001 20C8.31494 20 6.76549 18.4304 6.26653 17.7115C5.96463 17.2765 5.99806 16.7683 6.18066 16.4031C6.91705 14.9303 8.42234 14 10.069 14H13.7642C15.5134 14 17.1124 14.9883 17.8947 16.5528C18.0354 16.8343 18.0354 17.1657 17.8946 17.4473Z" fill="#152C70"></path> </g></svg>
            </button>
            {/* <TransitionGroup>
                {
                  items.map((item)=>(
                    <CSSTransition key={item} timeout={300} in={show} classNames="fade" unmountOnExit  >

                    </CSSTransition>
                  ))  
                }
            </TransitionGroup> */}
            <CSSTransition classNames="fade" nodeRef={nodeRef} timeout={300} in={show} unmountOnExit >
                <div ref={nodeRef} className='rounded-l-xl fixed top-24 right-0 bg-gray-800 text-white p-4 flex flex-col space-y-2'>
                    {items.map((item,idx)=>(
                        <button key={idx} className='rounded-xl overflow-hidden py-2 px-4 hover:bg-gray-500 bg-white  flex items-center space-x-2'>
                            <a className='hover:text-white' href={item.href}>{item.name}</a>
                        </button>   
                    ))}
                </div>
            </CSSTransition>
        </div>
    );
}

export default DropdownProfile;